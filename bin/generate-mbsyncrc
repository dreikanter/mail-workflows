...
# Called by bin/sync before running mbsync.
...
File.open(rc_path, "w", 0o600) do |f|
  accounts.each_with_index do |(name, acct), i|
    ...
    # Channel (pull-only, never modify remote)
    f.puts "Channel #{name}"
...
warn "Generated #{rc_path}"

# Ensure Maildir directories
...
warn "Maildir directories ready"
